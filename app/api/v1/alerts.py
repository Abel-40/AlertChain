...
@router.post("/create/", dependencies=[Depends(rate_limit(limit=10, window=60))])
...
@router.get("/", response_model=APIResponse[List[AlertOut]], dependencies=[Depends(rate_limit(limit=30, window=60))])
...
@router.patch("/{alert_id}/toggle", response_model=APIResponse[AlertOut], dependencies=[Depends(rate_limit(limit=20, window=60))])
...
@router.delete("/{alert_id}", dependencies=[Depends(rate_limit(limit=20, window=60))])
