...
@router.get("/", response_model=APIResponse[List[NotificationOut]], dependencies=[Depends(rate_limit(limit=30, window=60))])
...
@router.get("/unread-count", dependencies=[Depends(rate_limit(limit=60, window=60))])
...
@router.patch("/{notification_id}/read", response_model=APIResponse[NotificationOut], dependencies=[Depends(rate_limit(limit=30, window=60))])
...
@router.patch("/read-all", dependencies=[Depends(rate_limit(limit=10, window=60))])
...
@router.patch("/bulk/read", dependencies=[Depends(rate_limit(limit=10, window=60))])
...
@router.delete("/{notification_id}", dependencies=[Depends(rate_limit(limit=20, window=60))])
...
@router.delete("/bulk/", dependencies=[Depends(rate_limit(limit=10, window=60))])
